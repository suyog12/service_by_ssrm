...
async def get_billing_settings(
    ...
    async for db in get_tenant_db(schema):
        return await billing_service.get_or_create_billing_settings(
            db, schema, outlet_id
        )
...
async def update_billing_settings(
    ...
    async for db in get_tenant_db(schema):
        return await billing_service.update_billing_settings(
            db, schema, outlet_id, body.model_dump(exclude_none=True)
        )
...
async def list_bills(
    ...
    outlet_id = current_user["outlet_id"]
    async for db in get_tenant_db(schema):
        return await billing_service.list_bills(db, schema, outlet_id, status)
...
async def apply_discount(
    ...
    async for db in get_tenant_db(schema):
        return await billing_service.apply_discount(
            db, schema, outlet_id, bill_id,
            body.model_dump(), current_user["user_id"]
        )
...
async def get_bill_html(
    ...
    outlet_id = current_user["outlet_id"]
    async for db in get_tenant_db(schema):
        return await billing_service.get_bill_html(
            db, schema, outlet_id, bill_id
        )
...
async def list_credit_accounts(
    ...
    outlet_id = current_user["outlet_id"]
    async for db in get_tenant_db(schema):
        return await billing_service.list_credit_accounts(
            db, schema, outlet_id
        )
...
async def get_credit_account(
    ...
    outlet_id = current_user["outlet_id"]
    async for db in get_tenant_db(schema):
        return await billing_service.get_credit_account(
            db, schema, outlet_id, account_id
        )
